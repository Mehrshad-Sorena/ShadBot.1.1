...
class Chromosome:
	...
	def Get(
		...
		limits.elements['RSI_length_upper'] = int(optimizer_params['RSI_length'].iloc[-1] * 1.2)
		limits.elements['RSI_length_lower'] = int(optimizer_params['RSI_length'].iloc[-1] * 0.8)
		...
		for elm in Chromosome[chrom_counter].keys():
			...
			for rsi_elm in rsi_parameters.elements.keys():
				if rsi_elm == elm:
					rsi_parameters.elements[rsi_elm] = Chromosome[chrom_counter][elm]
		#//////////////////////////////////////

		return Chromosome, rsi_parameters, ind_parameters, pr_parameters, pr_config
	...
	def Initializer(
		...
		#************************** initialize Values ******************************************************
		rsiconfig = RSIConfig()
		path_society = rsiconfig.cfg['path_society'] + signalpriority + path_slash + signaltype + path_slash
		path_superhuman = rsiconfig.cfg['path_superhuman'] + signalpriority + path_slash + signaltype + path_slash
		...
		if os.path.exists(path_society + symbol + '.csv'):
			...
			while i < number_chromos:
				...
				rsi_parameters = self.RSIChromosomeInitializer()

				Chromosome_vares[i].update(rsi_parameters.elements)
	...
	def Creator(
		...
		Chromosome_vares[chrom_counter].update(rsi_parameters.elements)
	...
	def GraveyardCheker(
		...
		rsi_config = RSIConfig()
		path_graveyard = rsi_config.cfg['path_graveyard'] + signalpriority + path_slash + signaltype + path_slash
	...
	def RSIChromosomeInitializer(self):
	...
	def fucker_1(
		...
		Chromosome[chrom_counter]['RSI_apply_to'] = random.choice(apply_to_list)
	...
	def fucker_2(
		...
		rsi_period = randint(limits.elements['RSI_length_lower'], limits.elements['RSI_length_upper'])
		
		Chromosome[chrom_counter]['RSI_apply_to'] = random.choice(apply_to_list)
		Chromosome[chrom_counter]['RSI_length'] = rsi_period
		Chromosome[chrom_counter]['BestFinder_alpha'] = randint(limits.elements['BestFinder_alpha_lower'], limits.elements['BestFinder_alpha_upper'])/100
		Chromosome[chrom_counter]['BestFinder_alpha_low'] = randint(limits.elements['BestFinder_alpha_low_lower'], limits.elements['BestFinder_alpha_low_upper'])/100
		Chromosome[chrom_counter]['BestFinder_alpha_high'] = randint(limits.elements['BestFinder_alpha_high_lower'], limits.elements['BestFinder_alpha_high_upper'])/100
	...
	def ParameterOptimizer(self, path_elites_chromosome, alpha):
		...
		RSI_length = self.Finder(chromosome = chromosome, apply_to = 'RSI_length', alpha = alpha)
		limits.elements['RSI_length_upper'] = round(RSI_length['interval'][upper])
		limits.elements['RSI_length_lower'] = int(RSI_length['interval'][lower])
